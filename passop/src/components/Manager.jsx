...
const Manager = () => {
  ...
  const savePassword = () => {
    if (form.site.length > 3 && form.username.length > 3 && form.password.length > 3) {
      //to jab bhi naya pw aaye hume local storage aur password array dono ko modify karna padega
  ...
  const deletePassword = (id) => {
    ...
    if (c) {
      ...
      localStorage.setItem("passwords", JSON.stringify(passwordArray.filter(item => item.id !== id)))//local storage me bhi save kar do
  ...
  const editPassword = (id) => {
    ...
    //jisko edit karna hai form ko uss id ke equal kar do matlab vo upar display ho jayega
    setform(passwordArray.filter(i => i.id === id)[0])
    //pw array me og vale ko delete kar do, matlab niche se vo hat jayega aur fir hum firse save karenge
